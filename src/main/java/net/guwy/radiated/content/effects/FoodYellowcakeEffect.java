...
public class FoodYellowcakeEffect extends MobEffect {
    ...
    public void applyEffectTick(LivingEntity pLivingEntity, int pAmplifier) {
        ...
        if(rand < 0.33){
            pLivingEntity.hurt(new DamageSource("yellowcake_1"),Float.MAX_VALUE);
        } else if (rand < 0.66){
            pLivingEntity.hurt(new DamageSource("yellowcake_2"),Float.MAX_VALUE);
        } else {
            pLivingEntity.hurt(new DamageSource("yellowcake_3"),Float.MAX_VALUE);
        }
