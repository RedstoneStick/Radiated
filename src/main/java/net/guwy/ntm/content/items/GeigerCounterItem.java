    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, @NotNull Player pPlayer, InteractionHand pUsedHand) {
        if(!pLevel.isClientSide){
            pPlayer.getCapability(EntityRadiationProvider.ENTITY_RADIATION).ifPresent(handler -> {
                ...
                pPlayer.sendSystemMessage(Component.translatable("message.ntm.geiger_counter.1").withStyle(ChatFormatting.GOLD));


                component = Component.literal("").append(Component.translatable("message.ntm.geiger_counter.2").withStyle(ChatFormatting.YELLOW))
                        .append(Component.literal(Double.toString(handler.getLastGeigerVal())).append(" ").append(Component.translatable("tooltip.ntm.radiation.rad_s"))
                                .withStyle(getColorForEnvironmentRad(handler.getLastGeigerVal())));
                pPlayer.sendSystemMessage(component);

                component = Component.literal("").append(Component.translatable("message.ntm.geiger_counter.3").withStyle(ChatFormatting.YELLOW))
                        .append(Component.literal(Double.toString(handler.getPlayerRadiationVal())).append(" ").append(Component.translatable("tooltip.ntm.radiation.rad"))
                                .withStyle(getColorForPlayerRad(handler.getPlayerRadiationVal())));
                pPlayer.sendSystemMessage(component);

                component = Component.literal("").append(Component.translatable("message.ntm.geiger_counter.4").withStyle(ChatFormatting.YELLOW))
