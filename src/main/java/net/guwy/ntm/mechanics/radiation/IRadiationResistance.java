...
import net.guwy.ntm.utils.ItemTagUtils;
    ...
    static List<Component> DuctTapeTooltip(ItemStack pStack, List<Component> pTooltipComponents, int index){
        ...
        if(ductTapeAmount > 0 || getMaxApplicableDuctTape(pStack) > 0){
            ...
            if(currentMaxApplicableDuctTape > ductTapeAmount) {
                newTooltipComponents.add(Component.translatable("tooltip.ntm.armor.patching.1").withStyle(ChatFormatting.GRAY));
                newTooltipComponents.add(Component.translatable("tooltip.ntm.armor.patching.2").withStyle(ChatFormatting.GRAY));
            }

            newTooltipComponents.add(Component.literal(Component.translatable("tooltip.ntm.armor.patching.3").getString() + "[" + ductTapeAmount + "/" + currentMaxApplicableDuctTape + "]"));
