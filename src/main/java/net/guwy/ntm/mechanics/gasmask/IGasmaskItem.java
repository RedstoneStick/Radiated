...
public interface IGasmaskItem {
    ...
    static void setFilter(Player player, ItemStack filterItem, InteractionHand interactionHand){
        ...
        if(helmetItem.getItem() instanceof IGasmaskItem){
            ...
            // Else send non-replaceable filter message
                ...
                player.sendSystemMessage(Component.translatable("message.ntm.gas_mask.filter.fail.static_filter"));
            }
        }
        // If player isn't wearing a gas mask, send them a message
        else {
            player.sendSystemMessage(Component.translatable("message.ntm.gas_mask.filter.fail"));
        }
    }
    ...
    static List<Component> FilterTooltip(ItemStack pStack, List<Component> pTooltipComponents, int index){
        ...
        if(filterItem != Blocks.AIR.asItem()){
            ...
            newTooltipComponents.add(Component.translatable("tooltip.ntm.armor.gas_mask.warning"));
        }
