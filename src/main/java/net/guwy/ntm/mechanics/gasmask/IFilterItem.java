package net.guwy.ntm.mechanics.gasmask;
...
public interface IFilterItem {
    ...
    default List<Component> FilterItemTooltip(ItemStack pStack){
        ...
        if(!Screen.hasShiftDown()){
            pTooltipComponents.add(Component.translatable("tooltip.ntm.filter.generic"));
        } else {
            pTooltipComponents.add(Component.translatable("tooltip.ntm.filter.shift"));

            // Show filtration capabilities
            if(this.GetFiltrableHazards().contains(AirborneHazardTypes.AIRBORNE_PARTICLES)){
                pTooltipComponents.add(Component.translatable("tooltip.ntm.filter.shift.airborne_particles"));
            }
            if(this.GetFiltrableHazards().contains(AirborneHazardTypes.PARTICULATES)){
                pTooltipComponents.add(Component.translatable("tooltip.ntm.filter.shift.particulates"));
            }
            if(this.GetFiltrableHazards().contains(AirborneHazardTypes.CHEMICAL_GAS)){
                pTooltipComponents.add(Component.translatable("tooltip.ntm.filter.shift.chemical_gas"));
            }
            if(this.GetFiltrableHazards().contains(AirborneHazardTypes.BACTERIA_AEROSOL)){
                pTooltipComponents.add(Component.translatable("tooltip.ntm.filter.shift.bacteria_aerosols"));
            }
            ...
            if(this.GetFiltrableHazards().isEmpty()){
                pTooltipComponents.add(Component.translatable("debug.ntm.filter.no_filtration_value"));
            }
        }
