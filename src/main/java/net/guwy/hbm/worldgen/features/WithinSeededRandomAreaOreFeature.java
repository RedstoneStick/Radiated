...
public class WithinSeededRandomAreaOreFeature extends Feature<WithinSeededRandomAreaOreConfiguration> {
    public WithinSeededRandomAreaOreFeature(Codec<WithinSeededRandomAreaOreConfiguration> codec) {
        super(codec);
    }
     ...
     */
    @Override
    public boolean place(FeaturePlaceContext<WithinSeededRandomAreaOreConfiguration> context) {
        RandomSource randomsource = context.random();
        BlockPos blockpos = context.origin();
        WorldGenLevel worldgenlevel = context.level();
        WithinSeededRandomAreaOreConfiguration oreconfiguration = context.config();
    ...
    protected boolean doPlace(
            WorldGenLevel level,
            RandomSource random,
            WithinSeededRandomAreaOreConfiguration config,
            double minX,
            double maxX,
            double minZ,
            double maxZ,
                                            ...
                                            if (!bitset.get(l2)) {
                                                ...
                                                if (level.ensureCanWrite(blockpos$mutableblockpos)) {
                                                    ...
                                                    if (levelchunksection != null) {
                                                        ...
                                                        for (WithinSeededRandomAreaOreConfiguration.TargetBlockState oreconfiguration$targetblockstate : config.targetStates) {
                                                            if (canPlaceOre(
                                                                    blockstate,
    ...
    public static boolean canPlaceOre(
            ...
            RandomSource random,
            WithinSeededRandomAreaOreConfiguration config,
            WithinSeededRandomAreaOreConfiguration.TargetBlockState targetState,
        ...
        if (!targetState.target.test(state, random)) {
            ...
            Random seededRandom = new Random(level.getSeed() + config.seed_offset);
            int randX = (int) (seededRandom.nextGaussian() * config.maxDistFromSpawn);
            int randZ = (int) (seededRandom.nextGaussian() * config.maxDistFromSpawn);
            int range = config.range;

            if (mutablePos.getX() <= randX + range && mutablePos.getX() >= randX - range && mutablePos.getZ() <= randZ + randX && mutablePos.getZ() >= randZ - range){
