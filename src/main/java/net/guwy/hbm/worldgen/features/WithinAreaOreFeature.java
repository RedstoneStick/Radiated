...
public class WithinAreaOreFeature extends Feature<WithinAreaOreConfiguration> {
    public WithinAreaOreFeature(Codec<WithinAreaOreConfiguration> codec) {
        super(codec);
    }
     ...
     */
    @Override
    public boolean place(FeaturePlaceContext<WithinAreaOreConfiguration> context) {
        RandomSource randomsource = context.random();
        BlockPos blockpos = context.origin();
        WorldGenLevel worldgenlevel = context.level();
        WithinAreaOreConfiguration oreconfiguration = context.config();
    ...
    protected boolean doPlace(
            WorldGenLevel level,
            RandomSource random,
            WithinAreaOreConfiguration config,
            double minX,
            double maxX,
            double minZ,
            double maxZ,
                                            ...
                                            if (!bitset.get(l2)) {
                                                ...
                                                if (level.ensureCanWrite(blockpos$mutableblockpos)) {
                                                    ...
                                                    if (levelchunksection != null) {
                                                        ...
                                                        for (WithinAreaOreConfiguration.TargetBlockState oreconfiguration$targetblockstate : config.targetStates) {
                                                            if (canPlaceOre(
                                                                    blockstate,
    ...
    public static boolean canPlaceOre(
            ...
            RandomSource random,
            WithinAreaOreConfiguration config,
            WithinAreaOreConfiguration.TargetBlockState targetState,
            BlockPos.MutableBlockPos mutablePos
    ) {
        if (!targetState.target.test(state, random)) {
            return false;
        } else {
            if (mutablePos.getX() <= config.maxX && mutablePos.getX() >= config.minX && mutablePos.getZ() <= config.maxZ && mutablePos.getZ() >= config.minZ){
